...
import { getEffectiveModel } from "@/config/ai/types";
...
export const useGrammarStore = create<GrammarStore>((set, get) => ({
  ...
  checkGrammar: async (text: string) => {
    const provider = useAIConfigStore.getState().getActive();
    if (!provider) {
      toast.error("Provider AI belum dikonfigurasi");
      return;
    }
    ...
    try {
      ...
      try {
        ...
        if (!grammarErrors?.errors || grammarErrors.errors.length === 0) {
          set({ errors: [] });
          toast.success("Tidak ditemukan kesalahan tata bahasa");
          return;
        }
        ...
        if (preview) {
          ...
          grammarErrors.errors.forEach((error: GrammarError) => {
            // Mark only the exact error fragment to avoid fuzzy whole-sentence highlights
            markSingleError(marker, error);
          });
        }
      } catch (parseError) {
        toast.error(`Gagal mem-parse respons AI: ${parseError}`);
        set({ errors: [] });
      }
    } catch (error) {
      set({ errors: [] });
  ...
  dismissError: (index: number) => {
    set((state) => {
      ...
      const preview = document.getElementById("resume-preview");
      if (preview) {
        // Re-mark remaining errors after dismissal
        const marker = new Mark(preview);
        marker.unmark();

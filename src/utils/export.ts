  ...
  try {
    ...
    if (scaleMatch) {
      ...
      if (Number.isFinite(scale) && scale > 0 && scale < 1) {
        // Convert transform scale to zoom before server export to avoid pagination drift
    ...
    // Use PdfExport.tsx logic to unify width and padding handling
    clonedElement.style.setProperty("width", "100%", "important");
    ...
    // Inject style enhancements from PdfExport.tsx
    const styles = `
      ${capturedStyles}

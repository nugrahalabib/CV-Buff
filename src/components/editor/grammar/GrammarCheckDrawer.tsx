...
export function GrammarCheckDrawer() {
  ...
  const handleAccept = (index: number) => {
    ...
    // Recursively search and replace text in resume data
      ...
      for (const key in obj) {
        if (typeof obj[key] === "string") {
            ...
            if (error.context && obj[key].includes(error.context)) {
                 // Locate and replace the error text within the context only
                 const originalContext = error.context;
                 // Replace the error fragment with the suggestion inside the context
                 // Assumes the text appears once in context, or only replace the specific instance there
                 if (originalContext.includes(error.text)) {
                     const correctedContext = originalContext.replace(error.text, error.suggestion);
                     // Place the corrected context back into obj[key]
                     obj[key] = obj[key].replace(originalContext, correctedContext);
                     replaced = true;
                 } else {
                     // If text is not in context (unusual), suggestion is a fragment so direct
                     // context replacement would be wrong; do nothing here.
                 }
            }
            // If context is not found, fall back to global text match (riskier; only safe for unique/long strings)
                ...
                {errors.map((error, index) => (
                  ...
                  >
                    ...
                    <div className="px-4 py-3 border-b border-border/50 flex justify-between items-center bg-muted/20 rounded-t-xl">
                        ...
                        {/* Show reason only when different from Badge content */}
                        {error.reason && error.reason !== "Salah eja" && error.reason !== "Tanda baca" && (

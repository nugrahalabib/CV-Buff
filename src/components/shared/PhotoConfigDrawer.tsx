...
const PhotoConfigDrawer: React.FC<Props> = ({
  ...
  const handleFile = async (file: File) => {
    ...
    try {
      ...
      if (file.size > 2 * 1024 * 1024) {
        ...
        // If image is under 2MB, still do a light compression for performance
  ...
  const handleUrlChange = async (e: string) => {
    ...
    try {
      ...
      const checkImageSize = () => {
        return new Promise<void>((resolve, reject) => {
          fetch(proxyUrl, { method: "HEAD" })
            ...
            .catch(() => {
              // If size cannot be fetched, fall through to try loading image
              resolve();
            });
        });
      };
      ...
      toast.error(
        t("upload.invalidUrl", {
          defaultMessage: "URL gambar tidak valid atau tidak dapat diakses, coba URL lain",
        })
      );
      handleRemovePhoto();
    }

...
const ExportCard = ({
  ...
  return (
    ...
    >
      {/* Bottom ambient glow so icon color bleeds into the background */}
      <div className={cn(
      ...
      {/* Text content description (z-indexed above icon) */}
...
const PdfExport = ({ children }: { children?: React.ReactNode }) => {
  ...
  return (
    <Dialog open={isOpen} onOpenChange={(val) => {
      if (isLoading) return; // Prevent closing while exporting to avoid losing progress
      setIsOpen(val);
    }}>

...
export const TEMPLATE_PREVIEW_LOCALES = ["id", "en"] as const;
...
export const getTemplateSnapshotSrc = (
  manifest: TemplateSnapshotManifest | { locales: Record<string, Record<string, string>> },
  locale: TemplatePreviewLocale,
  templateId: string
) => (manifest.locales[locale]?.[templateId] as string | undefined) ?? null;
